...
//  double start = omp_get_wtime();
 ...
 if( argc < 2 ) {
  ...
  u    = createRealArray(omp_get_max_threads());	
  b    = createReal2DArray (m,m);	// The solution of the poisson?
  bt   = createReal2DArray (m,m);	// A matrix to store the transpose of b
  Z    = createReal2DArray (omp_get_max_threads(),nn);		// A zerovector that is used in sine transf.
...
*/
 ...
 // Make the solution umax
  ...
  for (j=0; j < m; j++) {
    for (i=0; i < m; i++) {
      if (b[j][i] > u[omp_get_thread_num()]) u[omp_get_thread_num()] = b[j][i];
    }
  }
  umax = u[0];
